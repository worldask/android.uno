	...
	touch: function(touch, cardCurrent) {
		...
		if (touch[0] != null) {
			...
			for (var i = cardCount - 1; i >= 0; i--) {
				// 点击位置有牌
				if (touch[0]._point.x > cards[i].getPositionX() - gc_cardWidth / 2 && touch[0]._point.x < cards[i].getPositionX() + gc_cardGap / 2
					&& touch[0]._point.y > cards[i].getPositionY() - gc_cardHeight / 2 && touch[0]._point.y < cards[i].getPositionY() + gc_cardHeight / 2) {
