var Button = cc.Layer.extend({
	...
	create: function(img, text, number) {
		...
		imgSprite.setPosition(cc.p(gc_size.width - gc_cardWidth * 1.8 - gc_cardWidth * 1.3 * number, gc_cardHeight * 1.4));
		
		// TODO 牌多时按钮会被挡住
		
		// 文字，固定为白色
		var txt = cc.LabelTTF.create(text, s_font1, s_fontsize1);
		imgSprite.addChild(txt);
		txt.setColor(cc.c4b(255, 255, 255, 255));
		
		// 文字居中
		txt.setAnchorPoint(0.5, 0.5);
		var posX = gc_cardWidth / 4;
		var posY = gc_cardHeight / 3;
